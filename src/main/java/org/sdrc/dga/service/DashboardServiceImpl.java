...
//	@Autowired
//	private AspirationalStateRepository aspirationalStateRepository;
		...
		if (lastVisitDataId == 0) {
			...
			if (areaLevelId > 3 && parentAreaId != -1) {
				...
				if (xform.getAreaLevel().getAreaLevelId() == Constants.DH_LEVEL) {
					// maxMinTimePeriodId =
					// lastVisitDataRepository.findMaxMinTimePeriodIdForADistrict(areaId,
					// formMetaId);
					maxMinTimePeriodId = lastVisitDataRepository.findAllTimePeriodIdForADistrict(areaId, formMetaId);
					if (maxMinTimePeriodId.get(0).toString().equals("2")
							&& maxMinTimePeriodId.get(2).toString().equals("2")) {
...
//					maxMinTimePeriodId = lastVisitDataRepository.findMaxMinTimePeriodIdForADistrictPHCCHC(areaId,
//							formMetaId);
					if (formMetaId == 6 || formMetaId == 7 || formMetaId == 8 || formMetaId == 9 || formMetaId == 10
							|| formMetaId == 5) {
						maxMinTimePeriodId = lastVisitDataRepository
								.findAllTimePeriodIdForADistrictPHCCHCForUhcAndHwc(areaId, formMetaId);
					} else {
...
//			maxMinTimePeriodId = lastVisitDataRepository.findMaxMinTimePeriodIdForAFacility(lastVisitDataId);
					...
					if (maxMinTimePeriodId.get(0)[0].toString().equals("3")
							&& maxMinTimePeriodId.get(0)[2].toString().equals("3")) {
						maxMinTimePeriodId = lastVisitDataRepository
								.findAllTimePeriodIdForAFacilityOfUhc(lastVisitDataId);
					}
				} else {
					maxMinTimePeriodId = lastVisitDataRepository.findAllTimePeriodIdForAFacilityOfUhc(lastVisitDataId);
				}
		...
		if (maxMinTimePeriodId.size() > 0) {
			...
			if (xform.getState().getAreaId() == 2 && formMetaId != 6 && formMetaId != 7 && formMetaId != 8
					&& formMetaId != 9 && formMetaId != 10 && formMetaId != 5) {
		...
		for (int timeperiodId : maxMinTime) {
			...
			if (lastVisitDataId == 0) {
				...
				// if areaID=0 then we will check if logged in user is of district Level
				if (areaLevelId > 3 && parentAreaId != -1) {
					areaId = collectUserModel.getUserRoleFeaturePermissionMappings().get(0)
							.getRoleFeaturePermissionSchemeModel().getAreaModel().getAreaId();
					...
					if (xform.getAreaLevel().getAreaLevelId() == Constants.DH_LEVEL) {
						...
						if (!spiderDatas.isEmpty())
							spiderDatas.addAll(
									facilityScoreRepository.findSpiderDataChartByFormIdForDistrictForDGA(formMetaId,
			...
			// if on google map pushpin is clicked
				...
				if (!spiderDatas.isEmpty())
					spiderDatas.addAll(facilityScoreRepository
							.findSpiderDataChartByLasatVisitDatAndFormIdAndTimePeriodId(formMetaId, lastVisitDataId,
									Integer.parseInt(spiderDatas.get(0)[3].toString()), timeperiodId));
			}
	...
	public List<FormXpathScoreMappingModel> getParentSectors(int timeperiodId, int stateId, int programId) {
		List<FormXpathScoreMapping> formXpathScoreMappings = formXpathScoreMappingRepository
				.findByParentXpathIdAndFormTimePeriodTimePeriodIdAndFormStateAreaIdAndFormProgramXFormMappingProgramProgramIdOrderByFormAreaLevelAreaLevelId(
						-1, timeperiodId, stateId, programId);
...
// if of national or guest or admin type then all the areas
	...
	public String exportToPdf(String spiderChart, String columnChart, Integer formId, Integer lastVisitDataId,
		...
		int stateId = xFormRepository.findByFormId(formId).getState().getAreaId();
...
//		url+=ctxPath;
		...
		HeaderFooter headerFooter = new HeaderFooter(url, Integer.toString(stateId));
...
//		List<TimePeriod> timePeriods = xFormRepository.findDistinctTimPeriodByStateAreaIdAndProgramId(stateId,programId);
		...
		if (stateId == 2233)
			timePeriods = xFormRepository.findDistinctTimPeriodByStateAreaIdAndProgramIdAndTimePeriodState2(stateId,
					programId);
		else
			timePeriods = xFormRepository.findDistinctTimPeriodByStateAreaIdAndProgramIdAndTimePeriodState(stateId,
					programId);
...
//		List<Area> states = areaRepository.findByAreaLevelAreaLevelIdOrderByAreaNameAsc(2);
	...
	@Override
	public AreaModel getState(int stateId) {
		Area area = areaRepository.findByAreaId(stateId);
		AreaModel model = new AreaModel();
		model.setAreaCode(area.getAreaCode());
		model.setAreaId(area.getAreaId());
		model.setAreaLevel(area.getAreaLevel().getAreaLevelName());
		model.setAreaLevelId(area.getAreaLevel().getAreaLevelId());
