...
public class DistrictHealthActionPlanServiceImpl implements DistrictHealthActionPlanService {
	...
	public boolean insertIntoDHAPConfiguratiion() {
		...
		try(FileInputStream fis = new FileInputStream(ResourceUtils.getFile("classpath:DHAP-Jk-Configuration.xlsx"))){
				...
				for (int row = 1; row <= sheet.getLastRowNum(); row++) {
					...
					while (cellIterator.hasNext()) {
						...
						case 9:
							...
							//put DH form id
							if((int)cell.getNumericCellValue() == 37)
								dhapConfiguration.setFaciltyType(FacilityType.DH);
							//put PHC formId
							else if((int)cell.getNumericCellValue() == 39){
								dhapConfiguration.setFaciltyType(FacilityType.PHC);
							//put CHC formid
							}else if((int)cell.getNumericCellValue() == 38){
...
//					set other properties here
					//change timePeriodId accodingly
					dhapConfiguration.setTimeperiod(timeperiodRepository.findByTimePeriodId(5));
	...
	public boolean aggregateDistrictHealthActionPlan() {
		try{
			//update formIds,timePeriodId,dh_formId accodingly
//			int[] formIds = {13,14,16};
			int[] formIds = {37,38,39};
			int timePeriodId = 5;
			int dh_formId = 37;
			for(int formId : formIds){
				List<DistrictHealthActionPlanConfiguration> dhapcs = dhapConfigurationRepository.findByFormFormIdAndTimeperiodTimePeriodId(formId, timePeriodId);
				for(DistrictHealthActionPlanConfiguration dhapc : dhapcs){
					if(dhapc.getStandard()!=null && !dhapc.getStandard().isEmpty() && dhapc.getFormula()!=null && !dhapc.getFormula().isEmpty()){
						...
						for(Object[] aggregatedData : allAggregatedData){
							...
							dhapAggDetail.setDistrict(formId!=dh_formId?aggregatedData[0].toString():aggregatedData[1].toString());
							dhapAggDetail.setBlock(formId!=dh_formId?aggregatedData[1].toString():"");
