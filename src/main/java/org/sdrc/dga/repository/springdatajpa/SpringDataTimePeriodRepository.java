...
public interface SpringDataTimePeriodRepository extends TimePeriodRepository{
	...
	@Override
	@Query (value="SELECT TOP 1 tp.* FROM TimePeriod tp where tp.stateId=:stateId ORDER BY tp.timePeriodId DESC", nativeQuery=true)
	TimePeriod findTop1ByStateidOrderByTimePeriodIdDesc(@Param("stateId") int stateId);
}
