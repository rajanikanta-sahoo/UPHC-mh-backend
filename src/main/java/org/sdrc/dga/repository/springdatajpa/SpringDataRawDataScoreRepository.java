...
public interface SpringDataRawDataScoreRepository extends
	...
	@Query(value="select sum(case when b.r1=b.r2 then 1 else 0 end) as sumval,b.ch1,b.ch2,b.tp "
			...
			+ "and area3_.AreaId=area2_.ParentAreaId "
			+ "and choicesdet4_.label=rawdatasco1_.score and (area3_.ParentAreaId=:districtId or area3_.AreaId=:districtId) and "
		...
		@Query(value="select sum(case when b.lvd1=b.lvd2 and b.score>=1 then 1 else 0 end) as AtleastOne, "
				...
				+ "and area4_.AreaId=area3_.ParentAreaId "
				...
				+ "(area4_.ParentAreaId=:districtId or area4_.AreaId=:districtId) and area14_.ParentAreaId=area3_.AreaId and "
		...
		@Query("SELECT SUM(CASE WHEN  rowRds.lastVisitData.lastVisitDataId = colRds.lastVisitData.lastVisitDataId AND rowRds.score >=1 AND colRds.score >=1 "
				...
				+ " rowRds.lastVisitData.timPeriod.timeperiod "
				+ " FROM RawDataScore rowRds ,"
				+ " RawDataScore colRds ,Area area, Area area2 "
				+ " WHERE colRds.rawFormXapths.xpath IN :xPathsIdCol "
				+ " AND rowRds.rawFormXapths.xpath IN :xPathsRow "
				+ " AND rowRds.lastVisitData.timPeriod.timePeriodId IN :timePeriodIds "
				+" 	AND rowRds.lastVisitData.lastVisitDataId=colRds.lastVisitData.lastVisitDataId"
				...
				+ " AND rowRds.lastVisitData.isLive = TRUE "
				+ " AND colRds.lastVisitData.isLive = TRUE "
				+ " AND area2.areaId=area.parentAreaId "
				+ " AND (area2.parentAreaId = :districtId OR area2.areaId=:districtId)"
		...
		@Override
		@Query(value=" SELECT * from RawDataScore where  xPathId = "
				+ "(SELECT rx.xPathId from RawFormXpaths rx  where rx.FormId=:formId and rx.label "
				+ "like '%Name of the Health facility%')",nativeQuery = true)
		public List<RawDataScore> findByLastVisitDataAndXpath(
				@Param("formId")Integer formId);
		
//		@Override
//		@Query("select *,(select xPathId from RawFormXpaths rf where rf.formId=" + 
//				"  (select formId from LastVisitData where lastVisitDataId=rd.lastVisitDataId)" + 
//				"   and rf.xpath=(select xpath from RawFormXpaths where xPathId = rd.xPathId)) as  newXPathId from RawDataScore rd" + 
//				"  where  lastVisitDataId in (select lastVisitDataId from LastVisitData where timePeriodId in :timePeriods)"
//				)
//		public List<Object[]> findRawDataAndXpathId(@Param("timePeriodIds")List<Integer> timePeriodIds);
//		
//		@Override
//		@Query("select * from RawDataScore where lastVisitDataId in (select lastVisitDataId from LastVisitData where timePeriodId in :timePeriods)"
//				)
//		public List<RawDataScore> findByTimePeriods(List<Integer> timePeriods);
