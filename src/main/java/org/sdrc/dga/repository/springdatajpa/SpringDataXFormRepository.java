...
public interface SpringDataXFormRepository extends XFormRepository, Repository<XForm, String> {
	...
	@Override
	@Query("Select DISTINCT(xform.timePeriod) FROM XForm xform   WHERE"
			+ " xform.isLive = True AND xform.state.areaId =:stateId  AND xform.programXFormMapping.program.programId = :programId ")
	List<TimePeriod> findDistinctTimPeriodByStateAreaIdAndProgramIdForRawData(@Param("stateId") int stateId,@Param("programId") int programId) ;
	
	
	@Override
	@Query("Select DISTINCT(xform.timePeriod) FROM XForm xform join  xform.formXpathScoreMappings fxm WHERE"
			+ " xform.isLive = True AND xform.state.areaId =:stateId AND fxm IS NOT NULL  AND xform.programXFormMapping.program.programId = :programId AND xform.timePeriod.stateId = :stateId")
	List<TimePeriod> findDistinctTimPeriodByStateAreaIdAndProgramIdAndTimePeriodState(@Param("stateId") int stateId,@Param("programId") int programId) ;
