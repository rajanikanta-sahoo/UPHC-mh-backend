...
public interface DevInfoTimeperiodRepository extends TimeperiodRepository,
	...
	@Override
	@Query("SELECT tp from UtTimeperiod tp where tp.stateId =:stateId")
	public List<UtTimeperiod> getByStateId(@Param("stateId")int stateId);
	
	@Override
	@Query("SELECT tp.stateId from UtTimeperiod tp where tp.timePeriod_NId =:timeperiodId")
	public Integer getStateId(@Param("timeperiodId")int timeperiodId);
